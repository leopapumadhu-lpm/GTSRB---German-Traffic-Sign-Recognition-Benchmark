...
# Voice phrases in different languages
voice_phrases = {
    'en': {
        'title': 'Voice Explanation',
        'button': '🎵 Listen to Explanation',
        'speaking': 'Speaking: ',
        'text': 'This traffic sign indicates: ',
        'confidence': ' with confidence '
    },
    'hi': {
        'title': 'ध्वनि व्याख्या',
        'button': '🎵 व्याख्या सुनें',
        'speaking': 'बोल रहा है: ',
        'text': 'यह यातायात संकेत दर्शाता है: ',
        'confidence': ' आत्मविश्वास के साथ '
    },
    'ta': {
        'title': 'குரல் விளக்கம்',
        'button': '🎵 விளக்கத்தைக் கேளுங்கள்',
        'speaking': 'பேசுகிறது: ',
        'text': 'இந்த போக்குவரத்து அடையாளம் காட்டுகிறது: ',
...
def get_voice_text(prediction, confidence, lang='en'):
    """Generate text for voice explanation"""
    confidence_text = f"{confidence:.1%}"
    
    if lang == 'en':
        return f"This traffic sign indicates {prediction}, with confidence {confidence_text}"
    elif lang == 'hi':
        return f"यह यातायात संकेत दर्शाता है {prediction}, आत्मविश्वास के साथ {confidence_text}"
    elif lang == 'ta':
        return f"இந்த போக்குவரத்து அடையாளம் காட்டுகிறது {prediction}, நம்பிக்கையுடன் {confidence_text}"
    else:
        return f"Sign: {prediction}, Confidence: {confidence_text}"
...
with st.sidebar:
    ...
    st.markdown("<div style='text-align: center;'><span class='feature-badge'>Top-K</span> <span class='feature-badge'>Multi-Lang</span> <span class='feature-badge'>Voice</span> <span class='feature-badge'>Visualize</span></div>", unsafe_allow_html=True)
...
if tab == "🏠 Dashboard":
    ...
    with col2:
        if uploaded_file and model:
            ...
            if st.session_state.predictions is not None:
                ...
                st.markdown(f"### 🔊 {voice_phrases[st.session_state.selected_lang]['title']}")
                
                # Generate voice text
                voice_text = get_voice_text(class_name, confidence, st.session_state.selected_lang)
                
                # Show what will be spoken
                st.info(f"**{voice_phrases[st.session_state.selected_lang]['speaking']}** {voice_text}")
                
                # Voice button
                if st.button(voice_phrases[st.session_state.selected_lang]['button'], use_container_width=True):
                    st.session_state.voice_played = True
                    st.success(f"🎤 Speaking in {language}: '{voice_text}'")
                    
                    # Note about browser TTS
                    st.info("💡 **Note**: For actual voice output, enable browser Text-to-Speech or use the built-in screen reader.")
                
                if st.session_state.voice_played:
                    # Instructions for voice
                    st.markdown("""
                    **🎧 How to hear the voice:**
                    1. Enable browser Text-to-Speech
                    2. Use screen reader (NVDA/JAWS on Windows, VoiceOver on Mac)
                    3. Copy the text and use any TTS app
                    """)
    ...
    **🎵 Voice Explanations**: Hear predictions spoken in your selected language.
    ...
    **German Traffic Sign Recognition Benchmark (GTSRB):**
    ...
    with col2:
        ...
        - [🔊 TTS Guide](https://developer.mozilla.org/en-US/docs/Web/API/SpeechSynthesis)
        """)
