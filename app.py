...
# ---------------- UI PAGES ----------------
menu = ["Home", "Upload & Predict", "Statistics", "About"]
choice = st.sidebar.selectbox("Menu", menu)

# ---------------- HOME PAGE ----------------
if choice == "Home":
    st.title("🚦 Welcome to GTSRB Traffic Sign Recognition App")
    st.write("""
    This app can recognize **German traffic signs** and provide:
    - Top-3 predictions
    - Sign meanings in English, Hindi, Tamil
    - Voice explanation
...
# ---------------- UPLOAD & PREDICT PAGE ----------------
    ...
    st.title("Upload Image or Use Webcam for Prediction")

    input_mode = st.radio("Input Mode:", ["Upload Image", "Webcam"])
    ...
    if input_mode == "Upload Image":
        ...
        st.info("Click 'Capture' to take a picture from webcam")
    ...
    # ---------------- PREDICTION ----------------
    if 'image' in locals():
        # Auto-cropping circle + digit-focused
        ...
        if circles is not None:
            ...
            x,y,r = circles[0][0]
            cropped_img = img_cv[y-r:y+r, x-r:x+r]
            image = Image.fromarray(cv2.cvtColor(cropped_img, cv2.COLOR_BGR2RGB))

        # Resize for model
        img_array = np.array(image.resize((30,30)))/255.0
        ...
        for i, idx in enumerate(top3_idx):
            meaning = class_names[idx]
            st.write(f"{i+1}. {meaning['en']} | {meaning['hi']} | {meaning['ta']} - Probability: {predictions[idx]:.2f}")

        # ---------------- VOICE OUTPUT ----------------
        voice_text = f"Top prediction: {class_names[top3_idx[0]]['en']}"
...
# ---------------- STATISTICS PAGE ----------------
    ...
    st.title("Prediction Probabilities Statistics")
    if 'predictions' in locals():
        prob_dict = {class_names[i]['en']: float(predictions[i]) for i in range(43)}
        st.bar_chart(prob_dict)
    else:
        st.info("Upload an image or capture from webcam to see statistics.")

# ---------------- ABOUT PAGE ----------------
elif choice == "About":
    st.title("About This App")
    st.write("""
    **GTSRB Traffic Sign Recognition App** built with Streamlit and TensorFlow.
    - Recognizes 43 German traffic signs
    - Shows meanings in multiple languages (English/Hindi/Tamil)
    - Provides top-3 predictions and voice explanation
